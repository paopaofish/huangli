...
class ExampleUnitTest {
  ...
  fun testTodayAlmanacAccuracy() {
    ...
    assertEquals("甲戌", lunar.ganZhiDay)
    assertEquals("马", lunar.zodiac)
    
    // Assert suitable and taboo lists for June 29, 2026 (定日)
    val expectedSuitable = listOf("祭祀", "祈福", "嫁娶", "订盟", "纳采", "冠笄", "裁衣", "合帐", "交易", "立券", "安床")
    val expectedTaboo = listOf("出行", "词讼", "治病", "移徙", "入宅", "栽种", "掘井")
    assertEquals(expectedSuitable, lunar.suitable)
    assertEquals(expectedTaboo, lunar.taboo)
    
    // Assert other daily predictions
    assertEquals("冲龙煞北", lunar.chongSha)
  ...
  fun testChineseNewYearAlmanacAccuracy() {
    ...
    assertEquals("壬戌", lunar.ganZhiDay)
    assertEquals("马", lunar.zodiac)
    
    // Assert suitable and taboo lists for CNY 2026 (成日)
    val expectedSuitable = listOf("嫁娶", "订盟", "纳采", "出行", "祭祀", "祈福", "动土", "破土", "安葬")
    val expectedTaboo = listOf("词讼", "治病")
    assertEquals(expectedSuitable, lunar.suitable)
    assertEquals(expectedTaboo, lunar.taboo)
    
    // Assert other daily predictions for 壬戌日
    assertEquals("冲龙煞北", lunar.chongSha)
  ...
  fun testTimezoneAndHourIndependence() {
    ...
    try {
      ...
      val gmt8_night = sdfGMT8.parse("2026-06-29 22:00:00")!!
      ...
      val la_night = sdfLA.parse("2026-06-29 22:00:00")!!
